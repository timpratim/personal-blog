...
const pkgImportPath = process.env['PKG_IMPORT_PATH'] ?? 'test-trebllewith-stainless/';
...
async function postprocess() {
  for await (const file of walk(path.resolve(__dirname, '..', '..', 'dist'))) {
    ...
    if (file.endsWith('.d.ts')) {
      ...
      // if we have `import { type Readable } from 'test-trebllewith-stainless/_shims/index'`,
